...
export const ensureDailyVendorEntries = async (
  userId: string,
  ...
  // console.log("User vendors snapshot", userVendorsSnapshot);
  const userVendors = userVendorsSnapshot.docs.map((vendorDoc) => ({
    ...(vendorDoc.data() as DailyVendor),
    id: vendorDoc.id,
    userId,
  }));
  // console.log("User vendors", userVendors);
      ...
      const existingExpenseSnapshot = await getDocs(
        query(
          ...
          where(
            "description",
            "==",
            `${vendor.vendorType} - ${vendor.vendorName}`,
          ),
